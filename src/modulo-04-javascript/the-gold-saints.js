...
function obterAlturaMediana() {
  ...
  if(alturas.length % 2 !== 0){
    ...
    return parseFloat(((alturas[metade-1] + alturas[metade]) /2).toFixed(2));
  }
}
...
function obterPesoMedio(){
  ...
  for(var i in goldSaints){
    ...
    contador++;
    soma += (goldSaints[i].pesoLb * 0.453592);
    }
  }
  return parseFloat((soma / contador).toFixed(2));
}
...
function obterPesoMedioDoadores(){
  ...
  for(var i in doadoresComPeso){
    soma += (doadoresComPeso[i].pesoLb * 0.453592);
  }
  return parseFloat((soma / doadoresComPeso.length).toFixed(2));
}
